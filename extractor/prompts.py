...
WEBSITE ELEMENTS (pre-scored by importance, higher score = more important):
Each element has an ID in square brackets — e.g. [el_3]. This ID is the
ONLY way to reference that element in your output.
{elements}

VALID ELEMENT IDS (use ONLY these, nothing else):
{valid_ids}
...
STRICT ID RULES (CRITICAL — violating these breaks the pipeline):
- The "elements" array in each scene MUST contain ONLY ids from VALID ELEMENT IDS above
- NEVER invent, guess, or generate an id that is not in that exact list
- NEVER output element text — ONLY the id string, e.g. "el_3", not "el_8a" or "About Me"
- Every id from VALID ELEMENT IDS must appear in EXACTLY ONE scene — no duplicates, no omissions
- If unsure where an id belongs, place it in the closest matching scene rather than dropping it
...
# ---------------------------------------------------------------------------
# Element Visibility Classifier — Groq (optional hybrid layer)
# ---------------------------------------------------------------------------
# Used ONLY for ambiguous edge cases the heuristics can't confidently
# resolve (e.g. element has valid bbox but extractor isn't sure if it's
# a meaningful content element vs decorative noise). This keeps token
# usage near-zero since 95% of elements are resolved by pure rules.

ELEMENT_RELEVANCE_PROMPT = """
You are a UI element classifier for a website-to-video system.

Given this element, decide if it is MEANINGFUL content (should appear in
a video) or NOISE (decorative, hidden helper, or structural junk).
...
# ---------------------------------------------------------------------------
# Camera Director v2 — aware of absolute document coordinates + page_height
# (use this once extractor_v4.py is integrated — adds page_height context
#  so Groq can reason about scroll position as a % of total page, not just
#  raw pixel Y, which is more robust across different page lengths)
# ---------------------------------------------------------------------------

CAMERA_DIRECTOR_SYSTEM_PROMPT_V2 = """
You are a professional film camera director for website demo videos.
Your ONLY job is to output camera instructions as strict JSON.
...
COORDINATE SYSTEM:
All Y values are ABSOLUTE DOCUMENT coordinates (distance from top of full
page), not viewport-relative. page_height tells you the full scrollable
height. Use scroll_percent = element_y / page_height to reason about
WHERE in the page this scene sits (top/middle/bottom), independent of
...
VISUAL CLUTTER RULE:
If 3+ elements are within 150px vertical range (absolute Y) → treat as a
group. Focus on group center, not individual elements. Use static or
slow zoom_in.
...
CINEMATIC MOTION RULES:
- zoom_in        : single focal element, hero, cta
- scroll_down    : elements spread > 200px vertically (absolute Y diff)
- pan_horizontal : elements spread > 300px horizontally
- static         : about sections, text-heavy, low element count
...
CAMERA BIAS RULE (viewport width = 1280px):
- focus.x < 400  → left
- focus.x > 880  → right
- else            → center
...
CAMERA_DIRECTOR_USER_PROMPT_V2 = """
SCENE DATA:
scene_id: {scene_id}
scene_index: {scene_index}
total_scenes: {total_scenes}
...
ELEMENTS (absolute document Y coordinates):
{elements_text}
